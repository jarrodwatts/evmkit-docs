...
EVM Kit uses thirdweb&rsquo;s [Wallet SDK](https://portal.thirdweb.com/wallet) to enable users to connect their wallets
...
Abstract away the wallet connection flow by creating wallets for user&rsquo;s under the hood.

Follow the [Local Wallet Documentation](https://portal.thirdweb.com/react/react.localwallet#usage-with-connectwallet).

### Account Abstraction

Create new smart wallets for user&rsquo;s that follow the EIP-4337 account abstraction standard and interact
...
View [Jarrod&rsquo;s full guide on setting up account abstraction](https://blog.jarrodwatts.com/i-fixed-web3-onboarding).
...
Inside an `.env.local` file, add your API key:
...
Inside an `.env.local` file, add your client ID:
